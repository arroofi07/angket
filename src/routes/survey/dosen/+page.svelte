<script lang="ts">
	...
	import { dosenSurvey } from '$lib/data/questions-dosen';
	import {
		surveyAnswers,
		currentStep,
		updateAnswer,
	...
	async function handleSubmit() {
		...
		try {
			...
			if (email) {
				await addBetaTester(email, 'dosen');
			}

			// Clear localStorage and redirect to thank you page
			resetSurvey('dosen');
			goto('/thank-you?type=dosen');
		} catch (error) {
...
<svelte:head>
	<title>Survei Dosen | Pra-Penelitian</title>
</svelte:head>
...
<div class="min-h-screen px-4 py-8">
	<div class="mx-auto max-w-2xl">
		...
		<div class="animate-fade-in mb-8 text-center">
			<a
				href="/survey"
				class="mb-4 inline-flex items-center text-gray-400 transition-colors hover:text-emerald-400"
			>
